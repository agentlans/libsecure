...
int sec_input_password(char* password, const char *prompt, size_t max_len) {
    if (!password || max_len == 0) return SEC_ERR_INVALID_ARGS;

    /* Initialize buffer using the core library function */
    sec_mem_wipe(password, max_len);
    ...
    while (status == 0) {
        render(prompt, password, current_len, cursor_pos, visible);
        ...
        switch (c) {
            ...
            case 127: case 8: /* Backspace */
                if (cursor_pos > 0) {
                    memmove(&password[cursor_pos - 1], &password[cursor_pos], current_len - cursor_pos + 1);
            ...
            case 27: { /* Escape sequence */
                ...
                if (next == '[') {
                    ...
                    if (code == 'D' && cursor_pos > 0) cursor_pos--;
                        ...
                        memmove(&password[cursor_pos], &password[cursor_pos+1], current_len - cursor_pos);
                        current_len--;
                    }
            ...
            default:
                if (c >= 32 && c <= 126 && (size_t)current_len < max_len - 1) {
                    memmove(&password[cursor_pos + 1], &password[cursor_pos], current_len - cursor_pos);
                    password[cursor_pos++] = (char)c;
                    current_len++;
